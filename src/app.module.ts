...
@Module({
  imports: [
    ...
    SequelizeModule.forRoot({
      dialect: 'postgres',
      host: process.env.NODE_ENV === 'test' ? (process.env.TEST_DB_HOST || '127.0.0.1') : (process.env.DB_HOST || 'localhost'),
      port: process.env.NODE_ENV === 'test' ? (Number(process.env.TEST_DB_PORT) || 5433) : (Number(process.env.DB_PORT) || 5432),
      username: process.env.NODE_ENV === 'test' ? (process.env.TEST_DB_USER || 'test_user') : (process.env.DB_USER || 'mainfix'),
      password: process.env.NODE_ENV === 'test' ? (process.env.TEST_DB_PASS || 'test_pass') : (process.env.DB_PASS || 'mainfix'),
      database: process.env.NODE_ENV === 'test' ? (process.env.TEST_DB_NAME || 'mainfix_test') : (process.env.DB_NAME || 'mainfix'),
      autoLoadModels: true,
      synchronize: false,
