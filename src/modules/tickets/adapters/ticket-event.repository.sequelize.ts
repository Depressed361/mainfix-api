...
export class SequelizeTicketEventRepository implements TicketEventRepository {
  constructor(
    @InjectModel(TicketEvent) private readonly model: typeof TicketEvent,
  ) {}
  async append(e: any) {
  ...
  async list(ticketId: string, p?: { page?: number; pageSize?: number }) {
    const rows = await this.model.findAll({
      where: { ticketId } as any,
      order: [['createdAt', 'DESC']],
      limit: p?.pageSize,
      offset: p?.page && p?.pageSize ? (p.page - 1) * p.pageSize : undefined,
    });
