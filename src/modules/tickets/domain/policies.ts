...
export function computeSlaTargets(
  createdAt: Date,
  priority: 'P1' | 'P2' | 'P3',
  sla: Record<'P1' | 'P2' | 'P3', { ackMinutes: number; resolveHours: number }>,
) {
  const ackDueAt = new Date(
    createdAt.getTime() + sla[priority].ackMinutes * 60 * 1000,
  );
  const resolveDueAt = new Date(
    createdAt.getTime() + sla[priority].resolveHours * 3600 * 1000,
  );
  return { ackDueAt, resolveDueAt };
}

export async function assertActorCanReadTicket(
  d: DirectoryQuery,
  actorUserId: string,
  ticket: TicketEntity,
) {
  const actor = await d.getUserMeta(actorUserId);
  if (actor.companyId !== ticket.companyId)
    throw new ForbiddenError('tickets.company_scope.denied');
}
...
export function assertValidStatusTransition(from: string, to: string) {
  ...
  if (!allowed[from] || !allowed[from].includes(to))
    throw new InvalidInputError('tickets.status.invalid_transition');
}
