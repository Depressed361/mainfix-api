import type { ApprovalRequestEntity, ApprovalRequestRepository, TicketEventCommand, UUID } from '../ports';
...
export class CreateApprovalRequest {
  ...
  async execute(actorUserId: UUID, p: { ticketId: UUID; reason?: string; amountEstimate?: string | null; currency?: string }): Promise<ApprovalRequestEntity> {
    const existing = await this.approvals.findPendingByTicket(p.ticketId);
    if (existing) return existing;
    const created = await this.approvals.create({ ticketId: p.ticketId, reason: p.reason ?? null, amountEstimate: p.amountEstimate ?? null, currency: p.currency ?? 'EUR' });
