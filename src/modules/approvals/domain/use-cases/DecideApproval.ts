import type { AdminScopeGuard, ApprovalRequestRepository, ContractsQuery, DirectoryQuery, TicketCommand, TicketEventCommand, UUID } from '../ports';
...
export class DecideApproval {
  constructor(
    ...
    private readonly tickets: { getTicketMeta(ticketId: UUID): Promise<{ companyId: UUID; siteId: UUID; buildingId?: UUID | null; contractVersionId: UUID }> },
  ...
  async execute(actorUserId: UUID, p: { approvalRequestId: UUID; decision: 'APPROVED' | 'REJECTED'; note?: string }) {
    ...
    if (!allowedRoles.has(actor.role)) throw new ForbiddenError('approvals.decision.role_forbidden');
    // Check admin scopes
    const companyOk = await this.guard.canAccessCompany(actorUserId, meta.companyId);
    const siteOk = await this.guard.canAccessSite(actorUserId, meta.siteId);
    const buildingOk = meta.buildingId ? await this.guard.canAccessBuilding(actorUserId, meta.buildingId) : false;
    if (!companyOk && !siteOk && !buildingOk) throw new ForbiddenError('approvals.decision.scope_forbidden');

    await this.approvals.setStatus(req.id, p.decision);
    const type = p.decision === 'APPROVED' ? 'APPROVAL_APPROVED' : 'APPROVAL_REJECTED';
    await this.events.appendEvent({ ticketId: req.ticketId, actorUserId, type, payload: { approvalRequestId: req.id, amountEstimate: req.amountEstimate, currency: req.currency, reason: req.reason, decisionNote: p.note ?? null } });
